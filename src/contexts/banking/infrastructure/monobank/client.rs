...
impl ProviderClient for MonobankClient {
    ...
    async fn register_webhook(&self, credential: &ProviderCredential, callback_url: &str) -> Result<(), ProviderFailure> {
        let response=self.client.post(format!("{}/personal/webhook",self.base_url.trim_end_matches('/'))).header("X-Token",credential.expose()).json(&serde_json::json!({"webHookUrl":callback_url})).send().await.map_err(|_|ProviderFailure::Classified{class:crate::contexts::banking::application::ProviderFailureClass::Transient})?;
        if response.status().is_success(){Ok(())}else{Err(ProviderFailure::Classified{class:super::MonobankAdapter::classify_status(response.status().as_u16())})}
    }
}
