...
impl SyncJob {
    pub fn request(
        ...
        if requested_from > requested_to {
            return Err(BankingError::InvalidValue("sync range is inverted"));
        }
        Ok(Self {
    ...
    pub fn claim(
        ...
        now: DateTime<Utc>,
        expires_at: DateTime<Utc>,
    ) -> Result<SyncLease, BankingError> {
        let holder = holder.into();
        if holder.is_empty() || expires_at <= now {
            return Err(BankingError::InvalidValue("invalid sync lease"));
        }
        if self.lease_expires_at.is_some_and(|expiry| expiry > now)
            && self.lease_holder.as_deref() != Some(holder.as_str())
        {
        ...
        self.fencing_token = self
            .fencing_token
            .checked_add(1)
            .ok_or(BankingError::InvalidValue("fencing token overflow"))?;
    ...
    pub fn complete_page(
        ...
        self.require_lease(lease, now)?;
        if processed.checked_add(quarantined) != Some(self.expected_page_events) {
    ...
    fn require_lease(&self, lease: &SyncLease, now: DateTime<Utc>) -> Result<(), BankingError> {
        if lease.fencing_token != self.fencing_token
            || self.lease_holder.as_deref() != Some(lease.holder.as_str())
            || self.lease_expires_at != Some(lease.expires_at)
            || lease.expires_at <= now
        {
            Err(BankingError::LeaseFenced)
        } else {
            Ok(())
        }
    }
    pub const fn id(&self) -> SyncJobId {
        self.id
    }
    ...
    pub const fn requested_to(&self) -> DateTime<Utc> {
        self.requested_to
    }
    pub const fn state(&self) -> SyncJobState {
        self.state
    }
