...
async fn correct_balance<U: LedgerUnitOfWork>(
    ...
    if command.expected_balance_version < 1 {
    ...
    let outcome = async {
        ...
        let (signed_before, balance_version) = tx
        ...
        if display_delta.is_zero() {
            return Err(LedgerError::invalid_money(
                "zero-delta correction is not a financial event",
            ));
        }
        let signed_delta = display_delta * Decimal::from(account.normal_sign());
        let equity = ensure_system(
            &mut tx,
            command.user_id,
            &account,
        ...
        tx.insert_correction_detail(CorrectionDetail {
            ...
            before_display_balance: display_before,
            target_display_balance: command.target_display_balance.amount(),
            display_delta,
            observed_balance_version: balance_version,
        ...
        let result = FinancialChangeResult {
            journal_entry_id: journal.id(),
            effects: vec![effect],
            replayed: false,
        };
...
async fn reverse_transaction<U: LedgerUnitOfWork>(
    ...
    let outcome = async {
        ...
        for posting in journal.postings() {
            *amounts.entry(posting.account_id()).or_default() += posting.signed_amount();
        }
...
async fn replace_transaction<U: LedgerUnitOfWork, C: CategoryCatalog>(
    ...
    let outcome = async {
        ...
        let replacement_account = accounts
            .iter()
            .find(|account| account.id() == command.account_id)
            .ok_or_else(LedgerError::not_found)?;
        ...
        let reversal_postings = original
            .postings
            .iter()
            .map(|posting| {
                Posting::rehydrate(
        ...
        let reversal = JournalEntry::post(
            JournalEntryId::generate(),
            command.user_id,
            format!("Replace: {}", command.description),
            PostingPurpose::Reversal,
            JournalSource::Correction,
            Actor::User(command.user_id),
            command.occurred_at,
        ...
        let system =
            ensure_system(&mut tx, command.user_id, replacement_account, role, clock).await?;
        ...
        let replacement = JournalEntry::post(
            ...
            command.causation_id,
            command.idempotency_key.clone(),
            JournalRelations::replacement_of(original.id),
            vec![
                Posting::for_account(
                    PostingId::generate(),
                    replacement_account,
        ...
        for (id, amount) in totals {
            effects.push(current_effect(&mut tx, nature_by_id[&id], amount).await?);
        }
        let result = ReplacementResult {
            reversal_journal_entry_id: reversal.id(),
            replacement_journal_entry_id: replacement.id(),
            effects,
            replayed: false,
        };
        store_result(
    ...
    match outcome {
        ...
        Err(error) => {
            tx.rollback().await?;
            replay_replacement_after(uow, &command, &request_hash, error).await
        }
    }
}
...
async fn current_effect<T: ProjectionStore>(
    tx: &mut T,
    account: &LedgerAccount,
    signed: Decimal,
) -> Result<AccountEffect, LedgerError> {
    let (balance, version) = tx
        .signed_balance(account.user_id(), account.id(), false)
        .await?
        .ok_or_else(LedgerError::not_found)?;
    Ok(AccountEffect {
        account_id: account.id(),
        ...
        display_effect: signed * Decimal::from(account.normal_sign()),
        signed_balance: balance,
        display_balance: balance * Decimal::from(account.normal_sign()),
...
async fn ensure_system<T: LedgerAccountStore>(
    ...
    if let Some(system) = tx
        .find_system_account(user_id, account.currency(), role, None)
        .await?
    {
        return Ok(system);
    }
...
async fn replay<T: CommandReceiptStore, R: DeserializeOwned>(
    tx: &mut T,
    user_id: crate::shared_kernel::UserId,
    scope: &str,
...
async fn store_result<T: CommandReceiptStore, R: Serialize>(
    tx: &mut T,
    user_id: crate::shared_kernel::UserId,
    scope: &str,
    key: &crate::shared_kernel::IdempotencyKey,
...
async fn finish<U: LedgerUnitOfWork>(
    uow: &U,
    tx: U::Tx<'_>,
    outcome: Result<FinancialChangeResult, LedgerError>,
    user_id: crate::shared_kernel::UserId,
    scope: &str,
    key: &crate::shared_kernel::IdempotencyKey,
    hash: &[u8; 32],
    ...
    match outcome {
        Ok(result) => match tx.commit().await {
            Ok(()) => Ok(result),
            Err(error) => replay_after(uow, user_id, scope, key, hash, error).await,
        },
        Err(error) => {
            tx.rollback().await?;
            replay_after(uow, user_id, scope, key, hash, error).await
        }
    }
}

async fn replay_after<U: LedgerUnitOfWork>(
    uow: &U,
    user_id: crate::shared_kernel::UserId,
    scope: &str,
    key: &crate::shared_kernel::IdempotencyKey,
