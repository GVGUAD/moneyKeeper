...
use super::{
    ...
    ports::{CommandReceiptStore, LedgerAccountStore, LedgerUnitOfWork, ProjectionStore, TransactionControl},
};
use super::super::{
    domain::{
...
async fn transfer<U: LedgerUnitOfWork>(
    ...
    if let Some(result) = replay(&mut tx, command.user_id, &command.idempotency_key, &request_hash).await? {
        tx.rollback().await?;
        return Ok(result);
    }

    let outcome = async {
        let locked = tx.lock_accounts(
            command.user_id,
            &[command.source_account_id, command.target_account_id],
        ).await?;
        if locked.len() != 2 { return Err(LedgerError::not_found()) }
        let source = locked.iter().find(|account| account.id() == command.source_account_id)
            .ok_or_else(LedgerError::not_found)?;
        let target = locked.iter().find(|account| account.id() == command.target_account_id)
            .ok_or_else(LedgerError::not_found)?;
        ...
        if same_currency {
            ...
            postings.push(Posting::for_account(
                PostingId::generate(), source, -command.source_amount.amount(), PostingPurpose::Ordinary,
            )?);
            postings.push(Posting::for_account(
                PostingId::generate(), target, command.target_amount.amount(), PostingPurpose::Ordinary,
            )?);
        } else {
            let rate = command.implied_rate.ok_or_else(|| {
                LedgerError::invalid_money("cross-currency transfer requires an implied rate")
            })?;
            if rate <= Decimal::ZERO { return Err(LedgerError::invalid_money("implied FX rate must be positive")) }
            let source_clearing = ensure_system(
                &mut tx, command.user_id, source, SystemAccountRole::FxClearing, clock,
            ).await?;
            let target_clearing = ensure_system(
                &mut tx, command.user_id, target, SystemAccountRole::FxClearing, clock,
            ).await?;
            postings.extend([
                Posting::for_account(PostingId::generate(), source, -command.source_amount.amount(), PostingPurpose::Ordinary)?,
                Posting::for_account(PostingId::generate(), &source_clearing, command.source_amount.amount(), PostingPurpose::Ordinary)?,
                Posting::for_account(PostingId::generate(), &target_clearing, -command.target_amount.amount(), PostingPurpose::Ordinary)?,
                Posting::for_account(PostingId::generate(), target, command.target_amount.amount(), PostingPurpose::Ordinary)?,
            ]);
        }
        ...
        if let Some(fee) = &command.fee {
            ...
            let expense = ensure_system(
                &mut tx, command.user_id, fee_account, SystemAccountRole::UncategorizedExpense, clock,
            ).await?;
            postings.push(Posting::for_account(
                PostingId::generate(), fee_account, -fee.amount.amount(), PostingPurpose::Ordinary,
            )?);
            postings.push(Posting::for_account(
                PostingId::generate(), &expense, fee.amount.amount(), PostingPurpose::Ordinary,
            )?);
        }
        ...
        let mut journal = JournalEntry::post(
            ...
            command.idempotency_key.clone(), JournalRelations::none(), postings,
        )?;
        if let Some(rate) = command.implied_rate { journal = journal.with_fx_rate(rate)?; }
        commit_journal(&mut tx, "transfer_funds", &journal, None, "ledger.entry-posted.v1").await?;
        ...
        for posting in journal.postings().iter().filter(|posting| {
            ...
            effects_by_account.entry(posting.account_id())
                .and_modify(|amount| *amount += posting.signed_amount())
                .or_insert(posting.signed_amount());
        }
        let mut effects = Vec::new();
        for account in [source, target] {
            let signed_amount = effects_by_account.get(&account.id()).copied().unwrap_or_default();
            let (signed_balance, balance_version) = tx
                .signed_balance(command.user_id, account.id(), false).await?
                .ok_or_else(LedgerError::not_found)?;
            effects.push(AccountEffect {
                account_id: account.id(), currency: account.currency().clone(), signed_amount,
                display_effect: signed_amount * Decimal::from(account.normal_sign()),
                signed_balance, display_balance: signed_balance * Decimal::from(account.normal_sign()),
                balance_version,
            });
        }
        let result = TransferResult {
            journal_entry_id: journal.id(), effects, implied_rate: journal.fx_rate(), replayed: false,
        };
        let result_json = serde_json::to_value(&result)
            .map_err(|error| LedgerError::persistence(error.to_string()))?;
        tx.insert_receipt(
            command.user_id, "transfer_funds", &command.idempotency_key,
...
async fn ensure_system<T: LedgerAccountStore>(
    ...
    if let Some(account) = tx.find_system_account(user_id, currency_account.currency(), role, None).await? {
        return Ok(account);
    }
    let account = LedgerAccount::open_system(
        LedgerAccountId::generate(), user_id, currency_account.currency().clone(), role, clock,
    );
...
fn validate(command: &TransferFunds) -> Result<(), LedgerError> {
    if command.source_account_id == command.target_account_id {
        return Err(LedgerError::invalid_state("source and target accounts must differ"));
    }
    for amount in [&command.source_amount, &command.target_amount] {
        if amount.is_zero() || amount.amount().is_sign_negative() {
            return Err(LedgerError::invalid_money("transfer amounts must be positive"));
        }
    }
    if command.fee.as_ref().is_some_and(|fee| fee.amount.is_zero() || fee.amount.amount().is_sign_negative()) {
...
async fn replay<T: CommandReceiptStore>(
    ...
    let Some(receipt) = tx.find_receipt(user_id, "transfer_funds", key, true).await? else {
        return Ok(None);
    };
...
async fn replay_after_failure<U: LedgerUnitOfWork>(
    ...
    let mut tx = uow.begin().await?;
    let replayed = replay(&mut tx, command.user_id, &command.idempotency_key, request_hash).await;
    tx.rollback().await?;
