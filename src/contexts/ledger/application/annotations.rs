...
async fn update_annotation<U: LedgerUnitOfWork, C: CategoryCatalog>(
    ...
    if let Some(receipt) = tx.find_receipt(
        command.user_id, "update_annotation", &command.idempotency_key, true,
    ).await? {
    ...
    let mut annotation = tx.find_annotation(command.user_id, command.journal_entry_id, true).await?
        .ok_or_else(LedgerError::not_found)?;
    annotation.update(
        command.changes, command.expected_version, Actor::User(command.user_id), clock.now(),
    )?;
    tx.save_annotation(&annotation).await?;
    let event = annotation.audit_events().last().cloned()
    ...
    tx.append_audit(&AuditRecord {
        event_id, user_id: command.user_id, aggregate_kind: "transaction_annotation",
        aggregate_id: annotation.id().into_uuid(), event_type: "ledger.annotation-changed.v1",
        actor_kind: "user", actor_reference: Some(command.user_id.to_string()),
        correlation_id: command.correlation_id.into_uuid(), payload: payload.clone(),
        occurred_at: command.occurred_at, recorded_at: event.changed_at,
    }).await?;
    tx.append_outbox(&integration_event(
        event_id, command.user_id, annotation.id().to_string(), annotation.version().get() as u64,
        "ledger.annotation-changed.v1", command.occurred_at, command.correlation_id, None, payload,
    )?).await?;
    let result = AnnotationResult {
        journal_entry_id: annotation.journal_entry_id(), version: annotation.version(), replayed: false,
    };
    let value = serde_json::to_value(&result).map_err(|error| LedgerError::persistence(error.to_string()))?;
    tx.insert_receipt(
        command.user_id, "update_annotation", &command.idempotency_key, &hash, &value, clock.now(),
    ).await?;
    tx.commit().await?;
    Ok(result)
}
