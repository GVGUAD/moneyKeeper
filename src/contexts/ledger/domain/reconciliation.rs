...
impl BalanceObservation {
    ...
    pub const fn provider_reported(&self) -> &Money {
        &self.provider_reported
    }
    pub const fn available(&self) -> Option<&Money> {
...
impl ReconciliationStatus {
    ...
    pub(crate) fn parse(value: &str) -> Result<Self, LedgerError> {
        match value {
            ...
            "ignored_older" => Ok(Self::IgnoredOlder),
            "approved" => Ok(Self::Approved),
            "dismissed" => Ok(Self::Dismissed),
            "stale" => Ok(Self::Stale),
            _ => Err(LedgerError::persistence(
                "stored reconciliation status is invalid",
            )),
        }
    }
}
...
impl ReconciliationCase {
    ...
    pub fn observe(
        ...
        Ok(Self {
            ...
            created_at: now,
            updated_at: now,
            events: vec![ReconciliationEvent::Observed {
                case_id: id,
                status,
            }],
        })
    ...
    pub fn approve(
        ...
        if self.status != ReconciliationStatus::Pending {
            return Err(LedgerError::invalid_state(
                "only a pending reconciliation can be approved",
            ));
        }
        ...
        self.updated_at = now;
        self.events.push(ReconciliationEvent::Approved {
            case_id: self.id,
            journal_entry_id,
        });
        Ok(())
    ...
    pub fn dismiss(
        ...
        if self.status != ReconciliationStatus::Pending {
            return Err(LedgerError::invalid_state(
                "only a pending reconciliation can be dismissed",
            ));
        }
        ...
        self.updated_at = now;
        self.events
            .push(ReconciliationEvent::Dismissed { case_id: self.id });
        Ok(())
    }
    ...
    pub(crate) fn mark_superseded(&mut self, now: DateTime<Utc>) -> Result<(), LedgerError> {
        if self.status != ReconciliationStatus::Pending {
            return Err(LedgerError::invalid_state(
                "only a pending reconciliation can be superseded",
            ));
        }
        ...
        self.updated_at = now;
        self.events
            .push(ReconciliationEvent::Superseded { case_id: self.id });
        Ok(())
    }
    ...
    pub const fn observation(&self) -> &BalanceObservation {
        &self.observation
    }
    pub const fn captured_ledger_balance(&self) -> &Money {
        &self.captured_ledger_balance
    }
    pub const fn captured_balance_version(&self) -> BalanceVersion {
        self.captured_balance_version
    }
    pub const fn delta(&self) -> &Money {
        &self.delta
    }
    pub const fn status(&self) -> ReconciliationStatus {
        self.status
    }
    pub const fn version(&self) -> ReconciliationVersion {
        self.version
    }
    pub const fn approval_journal_id(&self) -> Option<JournalEntryId> {
        self.approval_journal_id
    }
    pub const fn decision_actor(&self) -> Option<&Actor> {
