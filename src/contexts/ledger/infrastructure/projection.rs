...
use super::super::{domain::{LedgerAccountId, LedgerError}, public::ProjectionMismatch};

#[derive(Clone)]
pub(crate) struct PgLedgerProjection { pool: PgPool }
...
impl PgLedgerProjection {
    ...
    pub(crate) async fn verify(&self) -> Result<Vec<ProjectionMismatch>, LedgerError> {
        #[derive(FromRow)]
        struct Row { account_id: Uuid, user_id: Uuid, currency: String, projected: Decimal, posting_sum: Decimal }
        ...
        ).fetch_all(&self.pool).await.map_err(LedgerError::database)?;
        rows.into_iter().map(|row| Ok(ProjectionMismatch {
            account_id: LedgerAccountId::new(row.account_id), user_id: UserId::new(row.user_id),
            currency: CurrencyCode::new(row.currency).map_err(|_| LedgerError::persistence("stored currency invalid"))?,
            projected: row.projected, posting_sum: row.posting_sum,
    ...
    pub(crate) async fn rebuild(&self) -> Result<(), LedgerError> {
        ...
        sqlx::query("LOCK TABLE ledger.account_balances IN EXCLUSIVE MODE")
            .execute(&mut *tx).await.map_err(LedgerError::database)?;
        sqlx::query(
