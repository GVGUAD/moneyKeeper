...
impl SharingWorkflowWorker {
    ...
    pub async fn run_once(&self) -> anyhow::Result<SharingWorkflowReport> {
        ...
        let (operation, claim, bill_id, settlement_id) = match &work {
            SharingWorkflowWork::BillAccounting { claim, bill, .. } => {
                ("sharing.bill_accounting", claim, bill.id(), None)
            }
            SharingWorkflowWork::BillCancellation { claim, bill, .. } => {
                ("sharing.bill_cancellation", claim, bill.id(), None)
            }
            SharingWorkflowWork::SettlementAccounting { claim, settlement } => (
                "sharing.settlement_accounting",
                claim,
                settlement.bill_id(),
                Some(settlement.id()),
            ),
        ...
        let item_span = tracing::info_span!(
            ...
            operation,
            bill_id = %bill_id,
            settlement_id = tracing::field::Empty,
            correlation_id = %claim.correlation_id,
        );
        if let Some(settlement_id) = settlement_id {
            item_span.record("settlement_id", tracing::field::display(settlement_id));
        }
        item_span.in_scope(|| {
            tracing::info!(
