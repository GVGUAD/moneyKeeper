...
fn journal_rejects_too_few_zero_mixed_tenant_and_unbalanced_postings() {
    ...
    let cash = LedgerAccount::open_manual(
        LedgerAccountId::generate(), user, "Cash", CurrencyCode::new("UAH").unwrap(),
        AccountKind::Cash, AccountNature::Asset, &clock(),
    ).unwrap();
    let savings = LedgerAccount::open_manual(
        LedgerAccountId::generate(), user, "Savings", CurrencyCode::new("UAH").unwrap(),
        AccountKind::Savings, AccountNature::Asset, &clock(),
    ).unwrap();
    let other_cash = LedgerAccount::open_manual(
        LedgerAccountId::generate(), other, "Other", CurrencyCode::new("UAH").unwrap(),
        AccountKind::Cash, AccountNature::Asset, &clock(),
    ).unwrap();

    assert!(post_journal(user, vec![], JournalRelations::none()).unwrap_err().is_too_few_postings());
    assert!(Posting::for_account(
        PostingId::generate(), &cash, Decimal::ZERO, PostingPurpose::Ordinary
    ).unwrap_err().is_zero_posting());
    assert!(post_journal(
        user,
        vec![posting(&cash, "10.00", PostingPurpose::Ordinary), posting(&savings, "-9.00", PostingPurpose::Ordinary)],
        JournalRelations::none(),
    ).unwrap_err().is_unbalanced_journal());
    assert!(post_journal(
        user,
        vec![posting(&cash, "10.00", PostingPurpose::Ordinary), posting(&other_cash, "-10.00", PostingPurpose::Ordinary)],
        JournalRelations::none(),
    ).unwrap_err().is_tenant_mismatch());
}
...
fn journal_balances_each_currency_and_assigns_stable_positions() {
    ...
    let uah_a = LedgerAccount::open_manual(
        LedgerAccountId::generate(), user, "UAH A", CurrencyCode::new("UAH").unwrap(),
        AccountKind::Cash, AccountNature::Asset, &clock(),
    ).unwrap();
    let uah_b = LedgerAccount::open_manual(
        LedgerAccountId::generate(), user, "UAH B", CurrencyCode::new("UAH").unwrap(),
        AccountKind::Savings, AccountNature::Asset, &clock(),
    ).unwrap();
    let usd_a = LedgerAccount::open_manual(
        LedgerAccountId::generate(), user, "USD A", CurrencyCode::new("USD").unwrap(),
        AccountKind::Cash, AccountNature::Asset, &clock(),
    ).unwrap();
    let usd_b = LedgerAccount::open_manual(
        LedgerAccountId::generate(), user, "USD B", CurrencyCode::new("USD").unwrap(),
        AccountKind::Savings, AccountNature::Asset, &clock(),
    ).unwrap();
    let ids = [PostingId::generate(), PostingId::generate(), PostingId::generate(), PostingId::generate()];
    let drafts = vec![
        Posting::for_account(ids[0], &uah_a, Decimal::new(-4000, 2), PostingPurpose::Ordinary).unwrap(),
        Posting::for_account(ids[1], &uah_b, Decimal::new(4000, 2), PostingPurpose::Ordinary).unwrap(),
        Posting::for_account(ids[2], &usd_a, Decimal::new(-100, 2), PostingPurpose::Ordinary).unwrap(),
        Posting::for_account(ids[3], &usd_b, Decimal::new(100, 2), PostingPurpose::Ordinary).unwrap(),
    ];
    let journal = post_journal(user, drafts, JournalRelations::none()).unwrap();
    assert_eq!(journal.postings().iter().map(Posting::id).collect::<Vec<_>>(), ids);
    assert_eq!(journal.postings().iter().map(Posting::position).collect::<Vec<_>>(), vec![1, 2, 3, 4]);
}
...
fn journal_relations_are_construction_only_and_archived_repairs_are_allowed() {
    ...
    let counter = LedgerAccount::open_manual(
        LedgerAccountId::generate(), user, "Counter", CurrencyCode::new("UAH").unwrap(),
        AccountKind::Savings, AccountNature::Asset, &clock(),
    ).unwrap();
    cash.archive(AccountVersion::INITIAL, &clock()).unwrap();
    assert!(Posting::for_account(
        PostingId::generate(), &cash, Decimal::ONE, PostingPurpose::Ordinary
    ).unwrap_err().is_account_archived());
    let original = JournalEntryId::generate();
    let relations = JournalRelations::reversal_of(original);
    let journal = JournalEntry::post(
        JournalEntryId::generate(), user, "Repair", PostingPurpose::Reversal,
        JournalSource::Correction, Actor::User(user), clock().now(), clock().now(),
        CorrelationId::generate(), None, IdempotencyKey::new("repair").unwrap(), relations,
        vec![
...
fn annotations_are_versioned_without_touching_postings() {
    ...
    let mut annotation = TransactionAnnotation::new(
        AnnotationId::generate(), journal_id, user, "Coffee", None, None,
        NormalizedTags::empty(), BudgetVisibility::Included, clock().now(),
    ).unwrap();
    ...
    assert!(annotation.update(
        AnnotationChanges::default(), AnnotationVersion::INITIAL, Actor::User(user), clock().now()
    ).unwrap_err().is_version_conflict());
}
...
fn reconciliation_observation_is_visible_and_version_fenced() {
    ...
    let observation = BalanceObservation::new(
        ObservationId::generate(), source, amount("125.00", "UAH"), Some(amount("120.00", "UAH")),
        Utc.with_ymd_and_hms(2026, 8, 5, 9, 0, 0).unwrap(), 7, clock().now(),
    ).unwrap();
    let mut case = ReconciliationCase::observe(
        ReconciliationCaseId::generate(), user, account_id, observation,
        amount("100.00", "UAH"), BalanceVersion::new(4).unwrap(), Actor::System, clock().now(),
    ).unwrap();
    assert_eq!(case.status(), ReconciliationStatus::Pending);
    assert_eq!(case.delta().amount(), Decimal::new(2500, 2));
    assert!(case.approve(
        case.version(), BalanceVersion::new(5).unwrap(), JournalEntryId::generate(),
        Actor::User(user), clock().now(),
    ).unwrap_err().is_stale_observed_balance());
    assert_eq!(case.status(), ReconciliationStatus::Pending);

    let matched = ReconciliationCase::observe(
        ReconciliationCaseId::generate(), user, account_id,
        BalanceObservation::new(
            ObservationId::generate(), SourceReference::new("bank-feed", "resource-1", "balance-8").unwrap(),
            amount("100.00", "UAH"), None, clock().now(), 8, clock().now(),
        ).unwrap(),
        amount("100.00", "UAH"), BalanceVersion::new(4).unwrap(), Actor::System, clock().now(),
    ).unwrap();
