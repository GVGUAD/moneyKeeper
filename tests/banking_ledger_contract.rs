...
async fn mapping_existing_account_validates_public_ledger_contract_and_tenant() {
    ...
    let connection = banking
        .connect_provider(ConnectProvider {
            ...
            idempotency_key: IdempotencyKey::new("connect-mapping").unwrap(),
    ...
    let mapped = banking
        .bind_existing_resource(BindExistingResource {
            ...
            idempotency_key: IdempotencyKey::new("bind-existing").unwrap(),
            correlation_id: CorrelationId::generate(),
            requested_at: Utc::now(),
        })
        .await
    ...
    let non_comparable = banking
        .record_balance_observation(RecordBalanceObservation {
            user_id,
            connection_id: connection.id,
            resource_id,
            basis: BalanceBasis::CreditLimit,
            provider_money: Money::new(
            ...
            .unwrap(),
            sign_semantics: "provider_native".to_owned(),
            comparability: BalanceComparability::NotComparable(
                "not a scalar account balance".to_owned(),
            ),
            observed_at: observation_time,
            recorded_at: observation_time,
            correlation_id: CorrelationId::generate(),
    ...
    let rejected = banking
        .bind_existing_resource(BindExistingResource {
            user_id: other_user,
            resource_id,
            ledger_account_id: account.id,
            expected_resource_version: 2,
            idempotency_key: IdempotencyKey::new("cross-user").unwrap(),
            correlation_id: CorrelationId::generate(),
            requested_at: Utc::now(),
        })
        .await;
...
async fn create_and_map_is_retry_safe_and_opens_provider_observed_account() {
    ...
    let connection = banking
        .connect_provider(ConnectProvider {
            ...
            provider: "monobank".to_owned(),
            credential: ProviderCredential::new("token").unwrap(),
            idempotency_key: IdempotencyKey::new("connect-create-map").unwrap(),
            correlation_id: CorrelationId::generate(),
            requested_at: Utc::now(),
        })
        .await
    ...
    let make_command = || CreateAndMapResource {
        user_id,
        resource_id,
        account_name: "Reserve jar".to_owned(),
        expected_resource_version: 1,
        idempotency_key: IdempotencyKey::new("create-map").unwrap(),
        correlation_id: CorrelationId::new(Uuid::from_u128(77)),
        requested_at: Utc::now(),
    };
    let first = banking
        .create_and_map_resource(make_command())
        .await
        .unwrap();
    let replay = banking
        .create_and_map_resource(make_command())
        ...
        .unwrap();
    assert_eq!(
        first.mapping.ledger_account_id,
        replay.mapping.ledger_account_id
    );
    let account = ledger
        .get_account(user_id, first.mapping.ledger_account_id.unwrap())
        .await
        .unwrap();
    assert_eq!(
        account.authority,
        moneykeeper::contexts::ledger::public::AccountAuthority::ProviderObserved
    );
...
async fn provider_revisions_post_once_and_corrections_and_reversals_remain_visible() {
    ...
    let banking = banking::build_with_ledger(
        &verified,
        Arc::new(Aes256CredentialCipher::new("test-key", [8_u8; 32]).unwrap()),
        Arc::new(FixtureProvider),
        ledger.clone(),
        supporting.currencies,
        [1_u8; 32],
    ...
    let connection = banking
        .connect_provider(ConnectProvider {
            ...
            provider: "monobank".to_owned(),
            credential: ProviderCredential::new("token").unwrap(),
            idempotency_key: IdempotencyKey::new("connect-import").unwrap(),
            correlation_id: CorrelationId::generate(),
            requested_at: now,
        })
        .await
    ...
    banking
        .bind_existing_resource(BindExistingResource {
            ...
            ledger_account_id: account.id,
            expected_resource_version: 1,
            idempotency_key: IdempotencyKey::new("bind-import").unwrap(),
            correlation_id: CorrelationId::generate(),
            requested_at: now,
        })
        .await
        .unwrap();
    let add = |revision, state, amount, offset| IntakeProviderEvent {
        user_id: user,
        connection_id: connection.id,
        resource_id: resource,
        external_event_id: "event-stream".to_owned(),
        revision,
        state,
        operation_money: Money::new(amount, CurrencyCode::new("UAH").unwrap(), 2).unwrap(),
        description: "provider purchase".to_owned(),
        effective_at: now + Duration::seconds(offset),
        recorded_at: now + Duration::seconds(offset),
        correlation_id: CorrelationId::generate(),
    };
    let pending = banking
        ...
        .await
        .unwrap();
    moneykeeper::integration::process_managers::banking_import::import_provider_revision(
        &banking,
        &ledger,
        user,
