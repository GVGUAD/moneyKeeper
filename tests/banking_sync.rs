...
async fn banking_fixture() -> (
    ...
    let ids:Vec<Uuid>=sqlx::query_scalar("SELECT id FROM banking.external_resources WHERE user_id=$1 ORDER BY external_resource_id").bind(user_id.into_uuid()).fetch_all(&pool).await.unwrap();
    (banking,pool,user_id,connection.id,ids.into_iter().map(ExternalResourceId::new).collect())
}

fn intake(user_id:UserId,connection_id:ProviderConnectionId,resource_id:ExternalResourceId,revision:i64,amount:rust_decimal::Decimal)->IntakeProviderEvent{
    let now=Utc.with_ymd_and_hms(2026,8,20,12,0,0).unwrap();
    IntakeProviderEvent{user_id,connection_id,resource_id,external_event_id:"statement-event".to_owned(),revision,state:if revision==1{ProviderTransactionState::Pending}else{ProviderTransactionState::Settled},operation_money:Money::new(amount,CurrencyCode::new("UAH").unwrap(),2).unwrap(),description:"sanitized merchant".to_owned(),effective_at:now,recorded_at:now,correlation_id:CorrelationId::generate()}
}
...
async fn duplicate_revision_and_conflicting_content_are_distinguished_durably() {
    ...
    let first=banking.intake_provider_event(intake(user,connection,resources[0],1,dec!(-10.00))).await.unwrap();
    assert_eq!(first.outcome,ProviderEventIntakeOutcome::New);
    let duplicate=banking.intake_provider_event(intake(user,connection,resources[0],1,dec!(-10.00))).await.unwrap();
    ...
    let conflict=banking.intake_provider_event(intake(user,connection,resources[0],1,dec!(-12.00))).await.unwrap();
    assert_eq!(conflict.outcome,ProviderEventIntakeOutcome::ConflictingContent);
    ...
    assert_eq!(count,1);
    let state:String=sqlx::query_scalar("SELECT state FROM banking.provider_event_processes WHERE provider_event_id=$1").bind(first.provider_event_id.into_uuid()).fetch_one(&pool).await.unwrap();
...
async fn revision_identity_is_scoped_to_resource_and_new_facts_publish_ready_events() {
    let (banking,pool,user,connection,resources)=banking_fixture().await;
    let first=banking.intake_provider_event(intake(user,connection,resources[0],1,dec!(-10.00))).await.unwrap();
    let other=banking.intake_provider_event(intake(user,connection,resources[1],1,dec!(-10.00))).await.unwrap();
    let settled=banking.intake_provider_event(intake(user,connection,resources[0],2,dec!(-10.00))).await.unwrap();
